...
import ItemsBarHandler from './MathType/ItemsBarHandler_v2';
...
class MathType extends React.Component {
	...
	afterChanged(t) {
		if (!this.unMounted) {
			...
			if (n) {
				this.mathTagHandler.processLineTagNumbering()
				this.remoteCursorHandler.processUpdateRemoteCursorPositions()
			}
			this.theoremManagementHandler.processTheoremNumbering()
			this.captionNumberingHandler.processImageCaptionNumbering()
			this.captionNumberingHandler.processTableCaptionNumbering()
			this.selectionHandler.handleSelectionChanged(t, this.state)
			if (!this.props.readOnly) {
	...
	getRootEditStyle() {
		return this.props.rootEditorStyle
			? _.assignIn({}, this.pageSettingsHandler.getAsCssStyle(), this.props.rootEditorStyle)
			: this.pageSettingsHandler.getAsCssStyle();
	}
	renderMathEditContainer(readOnly) {
		...
		return readOnly
			? React.createElement("math-edit-container",
				{
	...
	render() {
		...
		if (this.error)
			return <math-type-error
				...
				<span>This document has been corrupted,please report to</span>
				<b>team@sorucep.com</b><span>for recovery,sorry for this issue.</span>
				<br />
				<span>Please send your<b>username</b> and <b>name</b> of this document.</span>
			</math-type-error>
		...
		return <math-type
			...
			onBlur={this.focusHandler.onMathTypeBlur}
		>
			...
			>
				<textarea
					...
					onBlur={(f) => { this.propsonBlur && this.props.onBlur(f); this.focusHandler.onHiddenInputBlur(f) }}
					onFocus={(f) => { this.props.onFocus && this.props.onFocus(f); this.focusHandler.onHiddenInputFocus(f) }}
				/>
			</hidden-input-wrapper>
			<hidden-input-wrapper
				ariaHidden={true}
			...
			<div style={{
				...
				{this.renderMathEditContainer(this.props.readOnly)}
