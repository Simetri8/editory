...
import {
	AppBar, Box, IconButton, Avatar, Icon, Toolbar, Typography,
...
class PageHeaderComponent extends React.Component {
	constructor(e) {
		...
		this.onSaveClick = event => {
			console.log("onSaveClick", this.props.saveStatus);
	...
	render() {
		var isReadOnly = !(this.isDocumentReadonly(this.props.activeDocumentOverview) || this.props.isAnonymousUser || this.props.isDocumentLoading);
		var documentNameStyle = {
			paddingTop: "10px"
		};
		if (MathGlobal.shouldUseSmallLayout()) {
			documentNameStyle.right = "40px";
			documentNameStyle.left = 90;
		}
		var requestNodeRename = undefined;
		MathGlobal.isMobileOrTablet() && isReadOnly && (requestNodeRename = () => {
			this.props.requestNodeRename(this.props.activeDocumentOverview.id);
		});

		return <AppBar onDoubleClick={this.handleDoubleClick} position="static">
			<Toolbar>
				<IconButton color="inherit"
					//onClick={() => this.props.history.push("/panel")}
					edge="start"
					component={Link}
					to="/panel">
				...
				<Box>
					...
					{/* <document-name onTouchStart={requestNodeRename}
                        onMouseDown={requestNodeRename}
                        style={documentNameStyle}>
                        <display-name>
                            <span title={this.props.activeDocumentOverview && this.props.activeDocumentOverview.name}>
                                <div
                                ...
                                {this.props.activeDocumentOverview
                                    ? this.props.activeDocumentOverview.name
                                    : ""}
                            </span>
                        </display-name>
                    </document-name> */}
				...
				{/* SAVE */}
				<IconButton
					onMouseDown={event => this.onSaveClick(event)}
					color="inherit"
					title={MathGlobal.isMac() ? "\u2318+S" : "Ctrl+S"}
					disabled={this.props.saveStatus != "unsave"}>
					<Icon>{this.props.isSaving ? 'cloud_upload' : 'save'}</Icon>
				</IconButton>
				...
				>
					<ListSubheader>{`Dışa Aktar`}</ListSubheader>

					<MenuItem onClick={(e) => { this.handleClose(e, { key: "save-as-html" }) }}>
						<ListItemIcon><Icon className="fa fa-file-code" /></ListItemIcon>Save as Html (.zip)
					</MenuItem>

					<MenuItem onClick={(e) => { this.handleClose(e, { key: "export-all" }) }}>
						<ListItemIcon><Icon className="fa fa-file-download" /></ListItemIcon>
                        Export Document to Latex</MenuItem>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "export-selection" }) }}>
						<ListItemIcon><Icon className="fa fa-file-download" /></ListItemIcon>
                        Export Selection to Latex</MenuItem>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "import" }) }}>
						<ListItemIcon><Icon>{` `}</Icon></ListItemIcon>
                        Import from Latex</MenuItem>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "import-from-zip" }) }}>
						<ListItemIcon><Icon className="fa fa-file-archive"></Icon></ListItemIcon>
						{`Import from .zip file`}
					</MenuItem>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "print" }) }}>
						<ListItemIcon><Icon>print</Icon></ListItemIcon>
						<ListItemText
							primary="Print"
							secondary={MathGlobal.supportSaveAsPDF() ? 'Save as PDF' : null}
						/>
						<ListItemSecondaryAction>
							{MathGlobal.isMac() ? "\u2318+P" : "Ctrl+P"}
						</ListItemSecondaryAction>

					</MenuItem>
					<Divider />
					<ListSubheader>{`Ayarlar`}</ListSubheader>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "show-tool-bar" }) }}>
						<ListItemIcon><Icon className="fa fa-file-code" /></ListItemIcon>show-tool-bar
					</MenuItem>

					<MenuItem onClick={(e) => { this.handleClose(e, { key: "page-setting" }) }}>
						<ListItemIcon><Icon className="fa fa-cogs" /></ListItemIcon>
                        Document Settings</MenuItem>
					<MenuItem onClick={(e) => { this.handleClose(e, { key: "user-settings" }) }}>
						<ListItemIcon><Icon className="fa fa-user-cog" /></ListItemIcon>
                        User Settings</MenuItem>

					{MathGlobal.licenseManagement()
						? <MenuItem onClick={(e) => { this.handleClose(e, { key: "license-dialog" }) }}>
							<ListItemIcon><Icon>{` `}</Icon></ListItemIcon>
							{'Lisans'}
						</MenuItem>
						: null}
					{!MathGlobal.isMac() && MathGlobal.isOfflineMode()
						? <MenuItem onClick={(e) => { this.handleClose(e, { key: "about-dialog" }) }}>
							<ListItemIcon><Icon>{` `}</Icon></ListItemIcon>
							{'Hakkında'}
						</MenuItem>
						: null}
